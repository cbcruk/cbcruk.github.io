...
**외부 문서를 요약해 넣으면 원문의 살이 따라 들어온다.** 압축 원칙은 CLAUDE.md "압축" 참고. 문체는 기준이 아니다 — 걸러야 할 것은 원천 없이 만들어진 값이다.

작성 직후 스스로 점검한다:

1. **출처** — 본문의 사실이 어디서 왔는가? 원천을 못 대는 값은 지어낸 것이다. 인용이면 `>`·`QuoteLink`로 표시한다.
...
- 원천 없는 값을 지어내지 않는다 — 모르면 질문으로 남긴다
