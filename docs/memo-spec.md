...
- **`kind: Recipe` + 번호 절차 3개 이상이면 `note`다.** 레시피(Type G)는 짧은 코드 조각을 여러 개 물고 있는데 절차가 산문으로 안 세져서, 코드 줄 수만 보면 `snippet` 으로 샌다. 코드가 절차의 부품이면 주인공이 아니다. 줄 수로 재지 않고 저자가 선언한 `kind` 를 키로 쓴다 — 코드만 있는 `Recipe`(`243`)는 절차가 없으니 그대로 `snippet` 이다.
